...
class JsonDiff {
  ...
  objectDiff (obj1, obj2) {
    ...
    for (const [key, value1] of Object.entries(obj1)) {
      if (key in obj2) {
        ...
        const [subscore, change] = this.diffWithScore(value1, value2)
        if (change) {
          result[key] = change
        } else if (this.options.showEqual) {
          result[key] = value1
        }
    ...
    if (Object.keys(result).length === 0) {
      [score, result] = [100 * Math.max(Object.keys(obj1).length, 0.5), this.options.showEqual ? {} : undefined]
    } else {
    ...
    for (const [key, candidate] of Object.entries(fuzzyOriginals)) {
      if (key !== '__next') {
        ...
        if (extendedTypeOf(item) === extendedTypeOf(candidate)) {
          const score = this.diffScore(item, candidate)
          if (!bestMatch ||
        ...
        case 'replace':
          if (!this.options.keysOnly) {
            ...
            for (i = i1, end5 = i2, asc5 = i1 <= end5; asc5 ? i < end5 : i > end5; asc5 ? i++ : i--) {
              change = this.diff(this.descalarize(seq1[i], originals1), this.descalarize(seq2[(i - i1) + j1], originals2))
              if (change) {
                result.push(['~', change])
                allEqual = false
              } else {
  ...
  diffScore (obj1, obj2) {
    ...
    const [score, change] = this.diffWithScore(obj1, obj2)
    return score
  }
...
function diff (obj1, obj2, options = {}) {
  return new JsonDiff(options).diff(obj1, obj2)
}
